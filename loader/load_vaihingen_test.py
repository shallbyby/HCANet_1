...
class vaihingenloader(Dataset):
    ...
    def __init__(self, root, split='train', crop_size=512, scale=False, rotate=True, HorizontalFlip=True, normalize=False, degree=10):
        ...
        for image_fp in os.listdir(os.path.join(self.root, self.split, 'image')):
            ...
            # Label path
            # top_mosaic_09cm_area1_0_0.png
            # top_mosaic_09cm_area1_noBoundary_0_0.png
            def search_n(s,c,n):
                r = s.find(c)
                while n > 1 and r >= 0:
                    r = s.find(c, r+1)
                    n -= 1
                return r
            index_ = search_n(image_fp,'_',4)
            def replace_char(string,char,index):
                string = list(string)
                string[index] = char
                return ''.join(string)
            label_fp = replace_char(image_fp,'_noBoundary_',index_)
            # label_fp = image_path.replace('_','_noBoundary_', num-1)
            # label_fp = image_fp.replace('RGB', 'label_noBoundary')
